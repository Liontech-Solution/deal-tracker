...
| A31e | Variantes con `honesty = 'suspicious'` y **`honestyBasis = 'declarado'`** | **no llevan umbral, y `trackedDays` bajo es lo normal** (`desmentidaPorLaTienda()`, mismo fichero): la evidencia es la tienda contradiciendo su propio mínimo de 30 días, no nuestra serie, así que acusa desde la primera pasada (#354). Aplicar aquí el umbral de A31d es un **P0 falso** — pasó en la validación de v0.5.0 sobre 199 variantes correctas (#430). Cuántas esperar y en qué tiendas, en **U26b/U26c/U26d** (`casos-ui.md`): no se repite la cifra aquí |
